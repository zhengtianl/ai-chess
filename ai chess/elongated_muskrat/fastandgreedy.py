    ...
    for hand, positions in own_tokens.items():
        if len(positions) > 0: # check that there are instances of the hand first
            ...
            for position in positions:
                ...
                score += (init_value+1)/min_target_dist
...
class Player:
    ...
    def action(self):
        ...
        upper_acts = list(self.board.available_actions("upper"))
        lower_acts = list(self.board.available_actions("lower"))
        
        # initialise best score and best move
        best_score = -inf
        
        if self.colour == "upper":
            best_move = upper_acts[0]
        else:
            best_move = lower_acts[0]
            
        for upper_act in upper_acts:
            for lower_act in lower_acts:
                anal_board = deepcopy(self.board)
                anal_board.update(upper_act, lower_act)
                score = eval_pos(anal_board.board, self.colour)
                if score > best_score:
                    if self.colour == "upper":
                        best_move = upper_act
                    else:
