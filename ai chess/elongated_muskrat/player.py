...
def eval_pos(board, colour):
    ...
    # assign values to own tokens as [#beatable]/[#identical^2 * closest enemy_dist]
    for hand, positions in own_tokens.items():
        if len(positions) > 0: # check that there are instances of the hand first
            ...
            for position in positions:
                ...
                score += init_value/(min_target_dist + 1)

    # subtract value opponent tokens on the board (higher weighted to incentivise captures)
